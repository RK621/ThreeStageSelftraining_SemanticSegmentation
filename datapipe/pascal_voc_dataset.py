...
pl_list = [] # shared by all
class PascalVOCAccessor (seg_data.SegAccessor):

    def __init__(self, ds, labels, mask, xf, pair, transforms, pipeline_type='cv', include_indices=False, pseudo_label=False, moco=False):
        ...
        if pseudo_label:
            if len(self.pl_list)==0:
                ...
                with zipfile.ZipFile(self.ds.pl_path, 'r') as zipf: # read all files at once 
                    def get_file(name):
                        with zipf.open(name) as ff:
                            img = Image.open(io.BytesIO(ff.read())); img.load(); tmp = np.array(img)
                            return tmp 
                    #
                    for i in range(self.__len__()):
                        cls = self.ds.sample_names[i] + '.png'
                        lab = get_file(cls)
                        self.pl_list[i] = lab 
    ...
    def get_pl_arr(self, sample_i): # additonal label loading (PL)
...
class PascalVOCDataSource (seg_data.DataSource):
    def __init__(self, n_val, val_rng, trainval_perm, fg_class_subset=None, augmented=False, pl_path=None):
        pascal_path = _get_pascal_path(exists=True)
        self.class_map = None

        #new 
        self.pseudo_label = (pl_path is not None)
        self.pl_path = pl_path # path to zip file 
