...
class SegAccessor (Dataset):
    ...
    def __getitem__(self, sample_i):
        ...
        if self.moco: # add a second image for moco
            if 'image_arr' in sample0: sample0['image_2nd_arr'] = sample0['image_arr'].copy()
            else: assert 0==1 # not implemented 
        ...
        if self.pseudo_label:  # pl 
            pl_arr = self.get_pl_arr(sample_i)
            if self.pipeline_type == 'pil':
                sample0['pl_pil'] = Image.fromarray(pl_arr)
            elif self.pipeline_type == 'cv':
                sample0['pl_arr'] = pl_arr.astype(np.int32)
            else:
                raise RuntimeError
...
class SegCollate (object):
    ...
    def _pad_sample(sample, size):
        ...
        if padding is not None:
            ...
            if 'pl' in sample:
                sample['pl'] = SegCollate._apply_padding_to_tensor(sample['pl'], padding, value=255) ## new 
            if 'mask' in sample:
