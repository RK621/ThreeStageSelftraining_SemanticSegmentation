...
class SegCVTransformPad (SegTransform):
    def pad_single(self, sample, min_size):
        ...
        if img_size[0] < min_size[0] or img_size[1] < min_size[1]:
            ...
            if 'pl_arr' in sample: # PL 
                sample['pl_arr'] = np.pad(sample['pl_arr'], [[h0, h1], [w0, w1]], mode='constant', constant_values=255)
            ...
            if 'image_2nd_arr' in sample:  # moco
                image2nd = np.append(sample['image_2nd_arr'][:, :, :3], alpha_channel, axis=2)
                sample['image_2nd_arr'] = np.pad(image2nd, [[h0, h1], [w0, w1], [0, 0]], mode='constant', constant_values=0)
    ...
    def pad_pair(self, sample0, sample1, min_size):
        ...
        if img_size0[0] < min_size[0] or img_size0[1] < min_size[1]:
            ...
            if 'pl_arr' in sample0: assert 0==1 # not implemented 
...
class SegCVTransformRandomCrop (SegCVTransformPad):
    ...
    def transform_single(self, sample):
        ...
        if 'pl_arr' in sample: # PL
            sample['pl_arr'] = sample['pl_arr'][pos[0]:pos[0]+self.crop_size[0], pos[1]:pos[1]+self.crop_size[1]]
        if 'mask_arr' in sample:
        ...
        if 'image_2nd_arr' in sample:
            pos2 = pos + np.round(self.crop_size//2 * self.rng.uniform(-1.0, 1.0, size=(2,))).astype(int)
            pos2 = np.minimum(extra, np.maximum(np.array([0,0]), pos2))
            sample['image_2nd_arr'] = sample['image_2nd_arr'][pos2[0]:pos2[0]+self.crop_size[0], pos2[1]:pos2[1]+self.crop_size[1]]
            #sample['image_2nd_arr'] = sample['image_2nd_arr'][pos[0]:pos[0]+self.crop_size[0], pos[1]:pos[1]+self.crop_size[1]]
...
class SegCVTransformRandomCropScaleHung (SegCVTransformPad):
    ...
    def transform_single(self, sample0):
        ...
        if 'pl_arr' in sample0: #PL
            pl_arr_i = sample0['pl_arr'][pos[0]:pos[0]+sc_size[0], pos[1]:pos[1]+sc_size[1]]
            sample0['pl_arr'] = cv2.resize(pl_arr_i, self.crop_size[::-1], interpolation = cv2.INTER_NEAREST)
        ...
        if 'image_2nd_arr' in sample0: # moco
            pos2 = pos + np.round(sc_size//2 * self.rng.uniform(-1.0, 1.0, size=(2,))).astype(int)
            pos2 = np.minimum(extra, np.maximum(np.array([0,0]), pos2))
            sample0['image_2nd_arr'] = sample0['image_2nd_arr'][pos2[0]:pos2[0]+sc_size[0], pos2[1]:pos2[1]+sc_size[1]]
            sample0['image_2nd_arr'] = cv2.resize(sample0['image_2nd_arr'], self.crop_size[::-1], interpolation=cv2.INTER_LINEAR)
...
class SegCVTransformRandomCropRotateScale (SegCVTransformPad):
    ...
    def transform_single(self, sample0):
        ...
        if 'image_2nd_arr' in sample: assert 0==1 # not implemented for moco
...
class SegCVTransformRandomFlip (SegTransform):
    ...
    def transform_single(self, sample):
        ...
        if 'pl_arr' in sample: 
            sample['pl_arr'] = self.flip_image(sample['pl_arr'], flip_flags_xyd)
        ...
        if 'image_2nd_arr' in sample:
            flip_flags_xyd_2nd = self.rng.binomial(1, 0.5, size=(3,)) != 0
            flip_flags_xyd_2nd = flip_flags_xyd_2nd & np.array([self.hflip, self.vflip, self.hvflip])
            sample['image_2nd_arr'] = self.flip_image(sample['image_2nd_arr'], flip_flags_xyd_2nd)
...
class colorjitter():
    ...
    def transform_single(self, sample):
        for i in ['image_arr', 'image_2nd_arr']: 
            if i in sample and np.random.uniform()<0.8:  # with probability 0.8 perform color jitter
                color_jitter = transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.4)
                #print('j shape1', sample[i].shape)
                j = Image.fromarray(sample[i])
                j = color_jitter(j)
                sample[i] = np.asarray(j)
                #print('j shape2', sample[i].shape)
...
class SegCVTransformNormalizeToTensor (SegTransform):
    ...
    def transform_single(self, sample):
        ...
        #print('dtype', sample['image_arr'].dtype, sample['image_arr'].shape)  # dtype uint8 (321, 321, 4) dtype uint8 (321, 321, 3)
        ...
        if image.shape[2] == 4:
            ...
            if 'image_2nd_arr' in sample:  # moco
                image2nd = img_as_float(sample['image_2nd_arr'])

                alpha_channel2nd = image2nd[:, :, 3:4]
                image2nd = image2nd[:, :, :3]
                if self.mean is not None and self.std is not None:
                    image2nd = (image2nd - (self.mean[None, None, :] * alpha_channel2nd)) / self.std[None, None, :]
                
                sample['image_2nd'] = image2nd.transpose(2, 0, 1).astype(np.float32)
                del sample['image_2nd_arr']
            ...
            if 'image_2nd_arr' in sample: # for moco 
                image2nd = img_as_float(sample['image_2nd_arr'])
                if self.mean is not None and self.std is not None:
                    image2nd = (image2nd - self.mean[None, None, :]) / self.std[None, None, :]
                sample['image_2nd'] = image2nd.transpose(2, 0, 1).astype(np.float32)
                del sample['image_2nd_arr']
        ...
        if 'pl_arr' in sample:
            sample['pl'] = sample['pl_arr'][None, ...].astype(np.int64)
            del sample['pl_arr']
...
class RandAugmentTransform:
    def __init__(self, cutoutsz=32):
        from apply_rand import rand_pascal
        self.ops = rand_pascal
        self.cutoutsz = cutoutsz

    def transform_single(self, sample):
        images = [sample['image_arr'].copy()]
        if 'labels_arr' in sample: images.append(sample['labels_arr'].copy()) 
        if 'pl_arr' in sample: images.append(sample['pl_arr'].copy())

        newimages = self.ops(images.copy(), cutoutsz=self.cutoutsz)

        sample['image_arr'] = newimages.pop(0)
        if 'labels_arr' in sample: sample['labels_arr'] = newimages.pop(0)
        if 'pl_arr' in sample: sample['pl_arr'] = newimages.pop(0)
        if 'image_2nd_arr' in sample: assert 0==1 # not implemented 
...
class RandAugmentTransformPost:
    def __init__(self, cutoutsz=32):
        from apply_rand import rand_pascal_index
        self.ops = rand_pascal_index
        self.cutoutsz = cutoutsz

    def transform_single(self, sample):

        image = sample['image'].transpose(1,2,0).astype(np.float32)
        jj = [np.min(image), np.max(image)]
        ii = image 
        ii = np.uint8((ii-jj[0])/(jj[1]-jj[0])*255.0)
        ii, indx, indy, msk = self.ops(ii.copy(),  cutoutsz=self.cutoutsz)
        ii = ii.transpose(2,0,1).astype('float32')/255.0*(jj[1]-jj[0])+jj[0]

        sample['image_post'] = ii 
        if 'pl' in sample: 
            tmp = sample['pl'][0].copy()[indx, indy]; tmp[msk<0.5] = 255; sample['pl_post'] = tmp[None,...]
        if 'labels' in sample:
            tmp = sample['labels'][0].copy()[indx, indy]; tmp[msk<0.5] = 255; sample['labels_post'] = tmp[None,...]
        if 'mask' in sample:
            tmp = sample['mask'][0].copy()[indx, indy]; tmp[msk<0.5] = 255; sample['mask_post'] = tmp[None,...]

        if 'image_2nd_arr' in sample: assert 0==1 # not implemented 

        sample['indx'] = indx[None,...]
        sample['indy'] = indy[None,...]
        sample['indmsk'] = msk[None,...]

            
        return sample


class RandAugmentTransformPostInplace:
    def __init__(self, cutoutsz=32):
        from apply_rand import rand_pascal_index
        self.ops = rand_pascal_index
        self.cutoutsz = cutoutsz
    ...
    def transform_single(self, sample):
        ...
        sample['image'] = ii 
        if 'pl' in sample: 
            tmp = sample['pl'][0].copy()[indx, indy]; tmp[msk<0.5] = 255; sample['pl'] = tmp[None,...]
        if 'labels' in sample:
            tmp = sample['labels'][0].copy()[indx, indy]; tmp[msk<0.5] = 255; sample['labels'] = tmp[None,...]
        if 'mask' in sample:
            tmp = sample['mask'][0].copy()[indx, indy]; tmp[msk<0.5] = 255; sample['mask'] = tmp[None,...]
