...
def load_dataset(dataset, n_val, val_seed, n_sup, n_unsup, split_seed, split_path, pl_path=None):
    val_rng = np.random.RandomState(val_seed)

    if pl_path is not None:  
        # only implemented pascal_aug and cityscapes
        assert dataset == 'cityscapes'  or dataset == 'pascal_aug'
    ...
    if dataset == 'pascal':
        ...
        ds_src = pascal_voc_dataset.PascalVOCDataSource(n_val=n_val, val_rng=val_rng, trainval_perm=trainval_perm, augmented=True, pl_path=pl_path)
        ds_tgt = ds_src
        val_ndx_tgt = val_ndx_src = ds_src.val_ndx
        test_ndx_tgt = ds_src.test_ndx
    elif dataset == 'cityscapes':
        ds_src = cityscapes_dataset.CityscapesDataSource(n_val=n_val, val_rng=val_rng, trainval_perm=trainval_perm, pl_path=pl_path)
        ds_tgt = ds_src
        val_ndx_tgt = val_ndx_src = ds_src.val_ndx
        test_ndx_tgt = ds_src.test_ndx
    else:
